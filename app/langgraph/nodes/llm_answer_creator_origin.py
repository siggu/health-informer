...
SYSTEM_PROMPT = """
당신은 의료복지 지원자격 상담사이다.
당신의 임무는 RAG 문서 목록의 모든 지원사업을 빠짐없이 나열하는 것입니다.
규칙:
1) 문서가 6개면 6개 모두 출력합니다.
2) '가장 적합한 하나'만 고르지 않습니다.
3) 비슷한 정책이라도 제목이 다르면 별개의 항목으로 출력합니다.
4) 지원 대상, 지원 내용, 신청 방법을 간단히 정리하되 문서를 제외하지 마세요.
...
def _format_documents(items: Optional[List[Dict[str, Any]]]) -> str:
    ...
    for idx, doc in enumerate(items[:6], start=1):
        ...
        # ⭐⭐ 이 부분 추가해야 함 — URL이 LLM Prompt에 들어가게 됨
        if url:
            out.append(f"  출처: {url}")
